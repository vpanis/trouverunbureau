class MangopayCollectionAccountWorker
  ...
  def init_log(mangopay_collection_account_id)
    str = 'MangopayPaymentAccountWorker on mangopay_collection_account_id: '
  ...
  def save_collection_account(account, coll_acc_data)
    ...
    @mca.update_attributes(mangopay_user_id: account['Id'], wallet_id: wallet['Id'],
      bank_account_id: bank_account['Id'], status: MangopayCollectionAccount.statuses[:accepted],
      mangopay_persisted: true, expecting_mangopay_response: false, active: true)
  end
