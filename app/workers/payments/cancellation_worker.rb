class CancellationWorker
  ...
  def more_than_a_month_cancellation(user_id)
    refund(ceil_2d(@booking.price * (1 - cancellation.penalty_fee)), user_id)
    # for the @booking.payment.update_attributes
    @booking.reload
    payout_to_user(floor_2d(@booking.price * cancellation.penalty_fee),
                   floor_2d(@booking.fee * cancellation.penalty_fee))
  end
  ...
  def refund(amount, user_id)
    ...
    Payments::Mangopay::RefundPayinWorker.perform_async(@booking.id, user_id, payout.id)
  end
