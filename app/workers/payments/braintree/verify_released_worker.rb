  ...
  module Braintree
    class VerifyReleasedWorker
      ...
      def init_log(payment_id)
        Rails.logger.info("Payments::Braintree::VerifyReleasedWorker on payment_id: #{payment_id}")
      end

      def invalid_escrow_status_for_release_log(transaction)
        str = 'Payments::Braintree::VerifyReleasedWorker invalid escrow status for verifing '
        str += "release on payment_id: #{@payment.id}, escrow_status: #{transaction.escrow_status}"
