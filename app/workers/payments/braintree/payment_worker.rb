  ...
  module Braintree
    class PaymentWorker
      ...
      def init_log(booking_id, payment_token, user_id, represented_id, represented_type)
        str = "Payments::Braintree::PaymentWorker on booking_id: #{booking_id}, "
        str += "payment_token: #{payment_token}, "
      ...
      def declined_log(booking_id)
        str = "Payments::Braintree::PaymentWorker declined_payment for booking_id: #{booking_id}"
      ...
      def compute_braintree_response(user_id, braintree_response)
        if braintree_response.success?
          ...
          Payments::Braintree::EscrowAcceptedWorker.perform_in(escrow_polling_time,
                                                               @booking.id, user_id)
        else
