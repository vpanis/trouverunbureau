  ...
  module Mangopay
    class FetchPayoutErrorWorker
      ...
      def save_payment_error(e)
        @mangopay_payment.update_attributes(error_message: e.to_s, status: 'PAYOUT_FAILED')
      end

      def fetch_transaction
        MangoPay::PayOut.fetch(@mangopay_payment.transaction_id)
      end
    end
  end
end
